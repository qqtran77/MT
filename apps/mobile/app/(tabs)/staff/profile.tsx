  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: '#f8fafc' }}>
      <View style={{ backgroundColor: '#1a3a5c', padding: 24, paddingTop: 60, alignItems: 'center' }}>
        ...
        <Text style={{ color: '#fff', fontSize: 20, fontWeight: '700' }}>{user?.fullName || 'Nhân viên'}</Text>
        ...
        <View style={{ backgroundColor: '#f59e0b', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 4, marginTop: 8 }}>
          <Text style={{ color: '#1a3a5c', fontSize: 12, fontWeight: '700' }}>
            {user?.role === 'staff' ? 'Nhân viên' : user?.role === 'branch_manager' ? 'Quản lý chi nhánh' : user?.role?.toUpperCase()}
          </Text>
        </View>
      </View>
